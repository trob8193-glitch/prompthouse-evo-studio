...
import { Download, Package, Box, Search, ExternalLink, Zap, Terminal, Code2 } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col h-full gap-6">
      <header className="flex flex-col gap-2">
        <h1 className="text-4xl font-black bg-gradient-to-r from-[#00f0ff] to-[#8a2be2] text-transparent bg-clip-text">
          Omni-Marketplace
        </h1>
        <p className="text-slate-400 text-sm">
          Universal format distribution hub. Tethering Antigravity, JetBrains, VS Code, Ollama, and Codex.
        </p>
      </header>

      <div className="flex gap-4 border-b border-slate-700 pb-2">
        {['all', 'ide', 'ai', 'raw'].map(tab => (
          <button
            key={tab}
        ...
        {filteredExts.map(ext => (
          <div key={ext.id} className="bg-slate-900/50 border border-slate-700/50 rounded-2xl p-6 flex flex-col gap-4 relative group hover:border-[#00f0ff]/50 transition-all hover:shadow-[0_0_30px_rgba(0,240,255,0.1)]">
            ...
            <div className="mt-4 pt-4 border-t border-slate-800 flex items-center justify-between">
              ...
              <button className="flex items-center gap-2 text-sm font-bold text-[#00f0ff] hover:text-white bg-[#00f0ff]/10 hover:bg-[#00f0ff]/30 px-4 py-2 rounded-lg transition-colors">
