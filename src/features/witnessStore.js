...
export const useWitnessStore = create((set, get) => ({
  ...
  logRealization: async (realization) => {
    ...
    try {
      await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/evolution/log-realization', {
        method: 'POST',
  ...
  runDoctorScan: async () => {
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/doctor/scan');
  ...
  triggerEvoDoctor: async (targetFiles = []) => {
    set({ is_healing: true });
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/doctor/heal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  triggerEvoEngineer: async () => {
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/engineer/evolve', { method: 'POST' });
  ...
  triggerEvoUIEngineer: async () => {
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/ui-engineer/evolve', { method: 'POST' });
      return await res.json();
    } catch (err) {
      return { success: false };
    }
