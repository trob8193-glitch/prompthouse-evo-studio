...
export function AutonomousSelfBuildCommandCenter() {
    ...
    const init = async () => {
      ...
      try {
        const [auditRes, implRes, evoRes] = await Promise.all([
          ...
          fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/self-implementation/status'),
          fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/evolution/autonomous/status')
        ]);
  ...
  async function runCycle() {
    ...
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/self-implementation/cycle', {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        ...
        try {
          const auditRes = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/audit/nuclear-truth');
          if (auditRes.ok) {
