export const LIVEFORGE_TEMPLATES = [
  {
    ...
    html: `<main class="app-shell"><section class="hero"><div class="badge">PromptHouse Evo</div><h1>Build your app from a prompt.</h1><p>Describe the app, choose a stack, and let Evo create a proof-gated blueprint.</p><button onClick={() => console.log('LiveForge template action triggered')}>Start Build</button></section><section class="cards"><article><h2>Prompt</h2><p>Turn your idea into a clear mission.</p></article><article><h2>Blueprint</h2><p>Generate screens, data, and tasks.</p></article><article><h2>Proof</h2><p>Block unverified shipping until proof exists.</p></article></section></main>`,
  ...
  },
  {
    ...
    html: `<main class="builder"><aside><h2>Blocks</h2><button onClick={() => console.log('LiveForge template action triggered')}>Brain</button><button onClick={() => console.log('LiveForge template action triggered')}>References</button><button onClick={() => console.log('LiveForge template action triggered')}>Tools</button><button onClick={() => console.log('LiveForge template action triggered')}>Rules</button><button onClick={() => console.log('LiveForge template action triggered')}>Proof</button></aside><section><div class="badge">Evo Studio Builder</div><h1>Make an agent like LEGO.</h1><div class="flow"><span>Brain</span><span>→</span><span>Tools</span><span>→</span><span>Proof</span><span>→</span><span>Publish</span></div></section></main>`,
  ...
  },
  {
    ...
    html: `<main class="side"><div class="badge">Browser Agent Bridge</div><h1>Capture this page.</h1><textarea ghostInput="Task: Turn this page into a PromptLink adapter."></textarea><button onClick={() => console.log('LiveForge template action triggered')}>Save to PromptBase</button><button onClick={() => console.log('LiveForge template action triggered')}>Create Proof Receipt</button></main>`,
