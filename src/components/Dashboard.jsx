    ...
    async function fetchLive() {
      try {
        const [statusRes, metricsRes, queueRes, nfRes] = await Promise.all([
          ...
          fetch(`${BRIDGE_URL}/api/metrics`),
          fetch(`${BRIDGE_URL}/api/queue/master`),
          fetch(`${BRIDGE_URL}/api/nightforge/status`),
        ]);
        ...
        setOps([
          { id: 'bridge',    type: 'Bridge',           desc: status ? `ONLINE v${status.version} • avg ${latency ?? '—'}ms • rps ${rps ?? '—'}` : 'OFFLINE', status: status ? 'ACTIVE' : 'OFFLINE' },
          { id: 'queue',     type: 'Execution Queue',  desc: `Items: ${queueCount}`, status: 'ACTIVE' },
          { id: 'nightforge',type: 'NightForge Daemon',desc: `Active: ${nfState.active ? 'YES' : 'NO'} • Running: ${nfState.running ? 'YES' : 'NO'}`, status: nfState.active ? 'ACTIVE' : 'IDLE' },
          { id: 'savings',   type: 'Cost Firewall',    desc: `Saved tokens: ${metrics?.firewall?.savedTokens ?? 0} • Saved $: ${metrics?.firewall?.savedDollars ?? '0.0000'}`, status: 'ACTIVE' }
        ]);
      } catch (e) {
        if (!mounted) return;
        setIq(null);
    ...
    >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        ...
        >
          ...
          <div className="absolute -right-10 -bottom-10 opacity-5"><Cpu size={240} /></div>
        </motion.div>
              ...
              {ops.map(op => (
                <div key={op.id} className="p-8 bg-black/40 rounded-3xl border border-slate-800/80 hover:border-indigo-500/50 transition-all duration-500 group">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className={`w-3 h-3 rounded-full ${op.status === 'ACTIVE' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]'} animate-pulse`} />
                  </div>
