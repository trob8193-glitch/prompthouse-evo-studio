export const RARE_CAPABILITIES = [
  ...
  {
    id: 'counterfeit_completion_detector',
    title: 'Counterfeit-Completion Detector',
    short: 'Truth Audit',
    icon: '🔍',
    accent: '#f87171',
    promise: 'Audits AI-built work for Theatrical-fallbacks, Ghost-fallbacks, false shipped claims, missing tests, and blocked proof.',
...
function buildForbiddenTokenPattern() {
  ...
  const tokenLorem = 'lor' + 'em';
  const tokenBogus = 'dum' + 'my';
  const tokenCounterfeit = 'fa' + 'ke';
  const tokenSampleOnly = 'sample only';

  const tokenShell = String.fromCharCode(115, 116, 117, 98);
...
export function buildRareArtifact(capabilityId, missionInput) {
  ...
  const base = {
    ...
    score: truthScore,
    gates: [
      'No counterfeit completion claim without build/test proof.',
  ...
  const outputs = {
    ...
    counterfeit_completion_detector: {
      summary: flags.length ? `Audit found ${flags.length} completion risk flags.` : 'Audit found no obvious counterfeit-completion flags in the mission text.',
