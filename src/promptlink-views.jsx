...
export function PromptLinkView() {
  ...
  const doHandshake = useCallback(async () => {
    ...
    if (result.connected) {
      ...
      // Load bridge data
      try {
        const caps = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || ((globalThis.process?.env?.BRIDGE_URL) || (globalThis.process?.env?.VITE_BRIDGE_URL) || (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001'))) + '/api/browser-bridge/forgecapsule').then(r => r.json());
