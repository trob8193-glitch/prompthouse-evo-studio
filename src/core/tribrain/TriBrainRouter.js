    ...
    return {
      ...
      brains: Object.fromEntries(
        Object.entries(this.brainSettings).map(([brain, state]) => [
          ...
          {
            ...state,
            capability: TRIBRAIN_CAPABILITY_MATRIX[brain],
          },
        ]),
      ),
    };
  ...
  route(command, permissionContext = {}) {
    ...
    if (!permission.allowed) {
      return createTriBrainResponseEnvelope({
        ...
        nextActions: ['Request approval or lower the command risk/mode before execution.'],
    ...
    const fallbackUsed = !online;

    return createTriBrainResponseEnvelope({
      commandId: command.id,
      respondingBrain: selectedBrain,
      truthState: fallbackUsed ? 'VERIFIED_WITH_WARNINGS' : 'VERIFIED',
      status: 'routed',
      summary: fallbackUsed
        ? `TriBrain routed to ${selectedBrain}, but selected brain is not fully available; fallback/local policy must handle execution.`
        : `TriBrain routed command to ${selectedBrain}.`,
      findings: fallbackUsed ? [{ severity: 'medium', module: 'TriBrainRouter', message: `${selectedBrain} is enabled but not currently available.` }] : [],
      evidence: {
        selectedBrain,
        selectedState,
        candidates,
      ...
      nextActions: fallbackUsed ? ['Use Studio Brain fallback or queue the command until the selected brain is available.'] : ['Execute through selected brain adapter and record proof.'],
