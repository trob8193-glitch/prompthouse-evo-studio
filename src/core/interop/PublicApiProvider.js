...
export class PublicApiProvider {
  ...
  async handleExternalRequest(apiKey, missionParams) {
    ...
    // PHYSICAL EXECUTION: No unverified reality execution. Real Gemini/OpenAI Call.
    if (process.env.OLLAMA_DAEMON_ACTIVE === 'true') {
      ...
      // execute result or route to local LLM if needed, here we just return success
      return {
        status: 'SUCCESS',
        truth_signed: true,
        sovereign_override: true,
        data: { message: "physical local execution via Sovereign Override" }
      };
    }
