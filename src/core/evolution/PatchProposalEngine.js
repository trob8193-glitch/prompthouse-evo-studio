...
function buildEnvBridgePatch(rootDir) {
  ...
  if (!current.includes("const BRIDGE_URL = ((globalThis.process?.env?.BRIDGE_URL) || (globalThis.process?.env?.VITE_BRIDGE_URL) || (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001'));")) return null;
  const proposedContent = current.replace(
    ...
    "const BRIDGE_URL = import.meta.env?.VITE_BRIDGE_URL || (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001');"
  );
  return {
    path: relPath,
    operation: 'update',
