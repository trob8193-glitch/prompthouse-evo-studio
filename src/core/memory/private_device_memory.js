...
const saveToServer = async (key, value) => {
    const encryptedValue = encrypt(value);
    const response = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/save', {
        method: 'POST',
...
const loadFromServer = async (key) => {
    const response = await fetch(`${(globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001')}/load/${key}`);
