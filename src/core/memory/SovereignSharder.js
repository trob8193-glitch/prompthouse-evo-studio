...
export class SovereignSharder {
  ...
  async shard(data, shardKey) {
    ...
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/memory/shard', {
        method: 'POST',
  ...
  async recall(query) {
    ...
    try {
      const res = await fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/memory/recall', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
