...
export class EvoCoreMasterOrchestrationKernel {
  ...
  brains() {
    ...
    return AUTHORITY_MAP.map(item => ({
      ...
      available: item.id === 'spinecore' || item.id === 'model-registry' || item.id === 'evo-diffuser' || Boolean(scripts[scriptMap[item.id]])
    }));
  }
  ...
  status() {
    ...
    return {
      ...
      rule: 'All autonomous studio work must produce receipts, use governed model/image routing, and pass Platform Sentinel before release claims.'
    };
  }
  ...
  planMission(mission = {}) {
    ...
    const route = text.includes('image') || text.includes('diffuse') || text.includes('visual')
      ? ['model-registry', 'evo-diffuser', 'cost-firewall', 'platform-sentinel']
      : text.includes('converge') || text.includes('amplify')
    ...
    const highRisk = /main branch|merge|deploy|provider|stripe|billing|auth|credential|readiness|dalle|openai|cloud/i.test(text);
    const receipt = {
      id,
