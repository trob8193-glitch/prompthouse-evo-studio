...
export function createRiftSession({ userId = 'studio-local-user', mode = 'evo-rift', evoName = 'local.rift.evo' } = {}) {
  ...
  db.prepare(`INSERT INTO rift_sessions (id, user_id, mode, status, truth_label, evo_route, evo_name, started_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`)
    .run(sessionId, userId, mode, RIFT_SESSION_STATUS.ACTIVE, TRUTH_LABELS.SANDBOX_ISOLATED, evoRoute, evoName, now);
  ...
  registerEvoRoute({ routeType: 'evo_url', routeValue: evoRoute, targetKind: 'rift_session', targetId: sessionId, truthLabel: TRUTH_LABELS.SANDBOX_ISOLATED });
...
export function getRiftSession(sessionId) {
  ...
  return row ? { ...row, truth_label: row.truth_label || TRUTH_LABELS.SANDBOX_ISOLATED } : null;
}
...
export function createTimelineBranch({ userId = 'studio-local-user', sessionId = null, title, choice = 'alternate path', assumptions = [] }) {
  const branchId = id();
  const graph = { nodes: [{ id: 'origin', label: 'Known current path', truth_label: TRUTH_LABELS.REAL }, { id: 'choice', label: choice, truth_label: TRUTH_LABELS.SANDBOX_ISOLATED }, { id: 'result', label: `Sandbox isolated result for ${choice}`, truth_label: TRUTH_LABELS.SANDBOX_ISOLATED }], edges: [{ from: 'origin', to: 'choice' }, { from: 'choice', to: 'result' }], assumptions };
  db.prepare('INSERT INTO rift_timeline_branches (id, user_id, session_id, title, truth_label, branch_graph_json) VALUES (?, ?, ?, ?, ?, ?)')
    .run(branchId, userId, sessionId, title || 'Untitled Rift Branch', TRUTH_LABELS.SANDBOX_ISOLATED, json(graph));
  return truthEnvelope({ label: TRUTH_LABELS.SANDBOX_ISOLATED, data: { id: branchId, graph } });
}
...
export function registerGridNode({ nodeName, nodeType, capabilities = [], status = 'configured', truthLabel = TRUTH_LABELS.SANDBOX_ISOLATED, boundaries = [] }) {
  const nodeId = id();
...
export function registerEvoRoute({ routeType, routeValue, targetKind, targetId = null, truthLabel = TRUTH_LABELS.SANDBOX_ISOLATED, metadata = {} }) {
  const routeId = id();
