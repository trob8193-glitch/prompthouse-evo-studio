...
const bridgeExpr = "((globalThis.process?.env?.BRIDGE_URL) || (globalThis.process?.env?.VITE_BRIDGE_URL) || (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001'))";
...
for (const file of allFiles) {
  ...
  // e.g. fetch((globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/...') -> fetch(bridgeExpr + '/api/...')
  content = content.replace(/['"`]http:\/\/(127\.0\.0\.1|localhost):3001(.*?)['"`]/g, (match, ip, rest) => {
    // If it's just (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001')
    if (!rest) return bridgeExpr;
    // If it's inside template literal `${(globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001')}/api/...`
    if (match.startsWith('\`')) {
      return `\`\${${bridgeExpr}}${rest}\``;
    }
    // If it's a string (globalThis.process?.env?.BRIDGE_URL || globalThis.process?.env?.VITE_BRIDGE_URL || 'http://127.0.0.1:3001') + '/api/...'
    return `${bridgeExpr} + '${rest}'`;
  });
