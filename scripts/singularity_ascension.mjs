...
function processPath(currentPath) {
  ...
  if (stats.isDirectory()) {
    ...
    if (['.js', '.jsx', '.ts', '.tsx', '.json', '.md', '.css', '.html', '.mjs', '.cjs'].includes(ext)) {
      ...
      if (modified) {
        ...
        Log.info(`[CONTENT] Updated: ${currentPath}`);
      }
    }
  }
  ...
  if (newName !== baseName) {
    ...
    Log.info(`[RENAME] Moved: ${baseName} -> ${newName}`);
  }
}

Log.info('🌌 Starting The Singularity Ascension...');
...
if (fs.existsSync(hiddenShards)) {
  ...
  Log.info(`[RENAME] Moved: .singularity-shards -> .singularity-shards`);
}
...
if (fs.existsSync(hiddenBrain)) {
  ...
  Log.info(`[RENAME] Moved: .singularity-brain.json -> .singularity-brain.json`);
}

Log.info('✅ Ascension Complete!');
