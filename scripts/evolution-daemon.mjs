...
async function callGeminiForEvolution(spatialData) {
  ...
  if (!apiKey) {
    console.error('\x1b[31m❌ No GEMINI_API_KEY found in .env\x1b[0m');
    return null;
  }
...
Focus on: spacing, alignment, contrast, visual hierarchy, micro-animations, premium feel.`;
  ...
  try {
    ...
    console.error(`\x1b[33m⚠️ Gemini call failed: ${e.message}. Trying OpenAI...\x1b[0m`);
  }
  ...
  if (openaiKey) {
    try {
      ...
      console.error(`\x1b[31m❌ OpenAI also failed: ${oe.message}\x1b[0m`);
    }
  }

  console.error('\x1b[31m❌ All AI providers exhausted.\x1b[0m');
  return null;
}
...
function applyCssChange(suggestion) {
  ...
  if (!fs.existsSync(cssPath)) {
    console.log(`\x1b[33m⚠️ Target CSS file not found: ${cssPath}\x1b[0m`);
    return false;
  }
  ...
  if (content.includes(suggestion.cssRule)) {
    console.log('\x1b[33m⚠️ This CSS rule already exists. Skipping.\x1b[0m');
    return false;
  }

  fs.writeFileSync(cssPath, content + marker, 'utf-8');
  console.log(`\x1b[32m✅ CSS applied to ${suggestion.targetFile}\x1b[0m`);
  return true;
}

async function runEvolution() {
  console.log('\n\x1b[35m═══════════════════════════════════════════════════\x1b[0m');
  console.log('\x1b[35m🧬 EVOLUTION DAEMON v2 — Real Gemini AI Evolution\x1b[0m');
  console.log('\x1b[35m═══════════════════════════════════════════════════\x1b[0m\n');

  // Step 1: Generate spatial map
  console.log('\x1b[36m[EVO] Step 1: Generating spatial map...\x1b[0m');
  let spatialData;
  try {
    spatialData = await generateSpatialMap();
  } catch (e) {
    console.log(`\x1b[33m⚠️ Spatial map capture requires a browser. Using component file analysis instead.\x1b[0m`);
  ...
  console.log('\x1b[36m[EVO] Step 2: Sending to Gemini AI for UI analysis...\x1b[0m');
  const suggestion = await callGeminiForEvolution(spatialData);

  if (!suggestion) {
    console.log('\x1b[31m❌ No suggestion received. Evolution cycle complete with no changes.\x1b[0m');
    return { evolved: false };
  }

  console.log(`\x1b[36m[EVO] Step 3: AI suggestion received:\x1b[0m`);
  console.log(`   Target: ${suggestion.targetFile}`);
  console.log(`   Change: ${suggestion.description}`);
  ...
  if (suggestion.cssRule) {
    ...
    console.log(`\x1b[33m[EVO] Component change suggested (requires manual review):\x1b[0m`);
    console.log(`   ${suggestion.componentChange}`);
  }
  ...
  console.log(`📋 Receipt: proof_receipts/evolution_logs/evo_${Date.now()}.json`);
