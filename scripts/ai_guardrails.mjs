...
export async function runGuardrails() {
  console.log('🛡️ [Guardrails] Running core repository scans...');
  let failed = false;

  const root = getProjectRoot();
  const { files } = await listFilesSafe(root, ['src', 'scripts', 'docs', 'tests', 'server'], {
    excludeGlobs: ['**/node_modules/**', '**/.git/**', '**/dist/**', '**/dist-electron/**'],
    excludePaths: [],
    includeExtensions: ['.js', '.jsx', '.json', '.md', '.mjs', '.cjs'],
    alwaysIncludeFiles: ['package.json', 'promptbridge-server.js']
  });

  const BAD_TERMS = [
    { regex: new RegExp('(?:^|\\n)(<<<<<<< |=======|>>>>>>> )'), name: 'Conflict Markers' },
    { regex: new RegExp('t' + 'odo:|d' + 'ummy|p' + 'laceholder code|f' + 'akeSuccess|m' + 'ockSuccess', 'i'), name: 'Placeholder Terms / Fake Success' },
    { regex: new RegExp('console\\.log\\(.*(API_KEY|SECRET).*\\)', 'i'), name: 'Secret Logging' },
    { regex: new RegExp('e' + 'mpty[\\s_]dashboard', 'i'), name: 'Missing Dashboard Shims' }
  ];

  for (const f of files) {
    const content = await fsPromises.readFile(f.fullPath, 'utf8');
    for (const term of BAD_TERMS) {
      if (term.regex.test(content)) {
        console.error(`❌ [Guardrails] Found ${term.name} in ${f.path}`);
        failed = true;
      }
    }
  }

  // Check Nightforge exports
  const nfContent = fs.existsSync(path.join(root, 'src', 'nightforge.js')) ? 
    fs.readFileSync(path.join(root, 'src', 'nightforge.js'), 'utf8') : '';
  const requiredNF = ['nightforgeState', 'runNightforgeCycle', 'buildNightforgeMetrics', 'updateNightforgeState', 'scheduleNightforgeDaemon', 'clearNightforgeDaemon'];
  for (const exp of requiredNF) {
    if (!nfContent.includes(exp)) {
      console.error(`❌ [Guardrails] Missing NightForge export: ${exp}`);
      failed = true;
    }
  }

  // Check for uncommitted dist-electron
  const gitignore = fs.existsSync(path.join(root, '.gitignore')) ? fs.readFileSync(path.join(root, '.gitignore'), 'utf8') : '';
  if (!gitignore.includes('dist-electron')) {
    console.error(`❌ [Guardrails] dist-electron missing from .gitignore`);
    failed = true;
  }

  if (failed) {
    console.error('❌ [Guardrails] FAILED! Please fix the errors above.');
    process.exit(1);
  } else {
    console.log('✅ [Guardrails] PASSED! No violations found.');
  }
}

// Execute if run directly
if (process.argv[1] && process.argv[1].endsWith('ai_guardrails.mjs')) {
  runGuardrails().catch(console.error);
}
