...
export class UniversalAIAdaptor {
  ...
  async chat(messages, options = {}) {
    ...
    const baseSequence = ['gemini', 'openai', 'ollama_qwen', 'ollama_llama3'];
    ...
    for (const p of sequence) {
      ...
      try {
        ...
        } else if (p === 'ollama' || p === 'ollama_qwen') {
          result = await this.callOllama(messages, { ...options, model: 'qwen3.6:latest' });
        } else if (p === 'ollama_llama3') {
          result = await this.callOllama(messages, { ...options, model: 'llama3:latest' });
        } else {
          continue;
        }
