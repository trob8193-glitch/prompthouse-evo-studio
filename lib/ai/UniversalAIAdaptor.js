...
import { getActiveModel, MODEL_PROVIDERS } from '../../src/core/ai/ModelRegistry.js';
...
export class UniversalAIAdaptor {
  ...
  async chat(messages, options = {}) {
    ...
    const requestedProvider = options.provider || activeModel.provider || this.getBestProvider();
    ...
    const baseSequence = ['gemini', 'openai', 'anthropic'];
  ...
  async callOllama(messages, options) {
    try {
      const model = options.model || 'qwen3.6:latest';
  ...
  async callAnthropic(messages, options) {
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model, max_tokens: 4096, system, messages: anthropicMessages })
      });
      const data = await response.json();
      if (data.error) throw new Error(data.error.message);
  ...
  async embed(text, options = {}) {
    if (this.keys.openai) {
      try {
        ...
        return { success: true, embedding: response.data[0].embedding, provider: 'openai' };
      } catch (e) {
        return { success: false, error: e.message, provider: 'openai' };
      }
    } else if (this.keys.gemini) {
      try {
        ...
        const response = await fetch(url, {
          ...
          headers: {
            'Content-Type': 'application/json',
            'x-goog-api-key': this.keys.gemini
          },
          body: JSON.stringify({ content: { parts: [{ text }] } })
        });
        const data = await response.json();
        if (data.error) throw new Error(data.error.message);
        return { success: true, embedding: data.embedding.values, provider: 'gemini' };
      } catch (e) {
        return { success: false, error: e.message, provider: 'gemini' };
      }
    }
    return { success: false, error: 'No provider available for embeddings (needs OpenAI or Gemini key).' };
  }
}
