...
export class PromptCompressor {
  constructor(ollamaUrl = 'http://localhost:11434') {
    ...
    this.model = 'qwen3.6'; // Fallback to standard model if 'evo-lm' is not found
  }

  async compress(text) {
    Log.info(`[COMPRESSOR] Attempting to compress ${text.length} chars.`);
    ...
    try {
      ...
      Log.info(`[COMPRESSOR] Compressed from ${text.length} to ${compressed.length} chars.`);
      return compressed;
    } catch (e) {
