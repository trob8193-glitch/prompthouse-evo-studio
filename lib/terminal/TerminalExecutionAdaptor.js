...
export class TerminalExecutionAdaptor {
  constructor(workspaceRoot, sandboxMode = false) {
    ...
    if (this.sandboxMode) {
      const licenseManager = new EnterpriseLicenseManager(this.workspaceRoot);
      licenseManager.enforceEnterprise('The Phantom Sandbox');
      
      this.phantomRoot = path.join(this.workspaceRoot, '.phantom-sandbox');
      if (!fs.existsSync(this.phantomRoot)) {
