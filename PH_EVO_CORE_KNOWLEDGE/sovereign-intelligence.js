...
class SovereignProtocol {
  ...
  async invokeAgent(purpose, input) {
    try {
      const response = await fetch(`${BRIDGE}/api/agents/invoke`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ agentId: purpose, input })
      });
      const data = await response.json();
      return data.output;
    } catch (e) {
      console.error(`[SovereignAgent] Failed to invoke ${purpose}:`, e.message);
      return null;
    }
  }
  ...
  async execute() {
    ...
    if (gaps.length > 0) {
      console.log('  → Consulting Evo Lead for strategic prioritization...');
      const strategy = await this.invokeAgent('Evo Strategic Lead', `I found ${gaps.length} gaps. Prioritize these and suggest a build order: ${JSON.stringify(gaps.slice(0, 5))}`);
      if (strategy) console.log(`  [STRATEGY] ${strategy.substring(0, 100)}...`);
    }
