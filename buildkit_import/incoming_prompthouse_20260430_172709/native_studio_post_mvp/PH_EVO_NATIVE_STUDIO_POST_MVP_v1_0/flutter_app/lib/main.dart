...
class PromptHouseEvoNativeStudio extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(brightness: Brightness.dark, colorSchemeSeed: const Color(0xFFF5B942), useMaterial3: true),
...
class _StudioHomePageState extends State<StudioHomePage> {
  ...
  void _forge() {
    ...
    final m = StudioMission(
      ...
      modules: const ['Prompt Architect', 'Product Framer', 'Tool Router', 'Truth Auditor', 'Artifact Builder'],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [Center(child: Padding(padding: const EdgeInsets.only(right: 12), child: Chip(label: Text(mission?.status.label ?? 'intake'))))],
      ),
      body: ListView(
        padding: const EdgeInsets.all(18),
        children: [
          _Card(title: '🦁 Post-MVP Mobile Dev Studio', child: const Text('Prompt Auth → PromptBase → Fire Orders → Blueprint → Proof Deck → Bot Customers → Repair Loop')),
...
class _Card extends StatelessWidget {
  ...
  final Widget child;
  @override
  Widget build(BuildContext context) => Card(margin: const EdgeInsets.only(bottom: 14), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 10), child])));
}
