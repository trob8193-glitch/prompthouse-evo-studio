...
const PANELS = [
  { id: 'home', label: 'Home Cockpit', icon: Home, color: 'text-neon-cyan' },
  { id: 'capture', label: 'Reality Capture', icon: Camera, color: 'text-emerald-400' },
  { id: 'bot_cast', label: 'Bot Cast Roster', icon: Users, color: 'text-neon-cyan' },
...
export const ExtensionCockpitView = () => {
  ...
  const PanelItem = ({ panel }) => {
    ...
    return (
      ...
      >
        <div className={`p-2 rounded-2xl glass-extreme border-neon-glow ${panel.color}`}>
          <Icon size={18} />
        </div>
        <span className={`text-sm font-bold ${isActive ? 'text-white' : 'text-slate-400'}`}>{panel.label}</span>
        {isActive && <ChevronRight size={16} className="ml-auto text-neon-cyan" />}
      </button>
    );
  };
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-[600px]">
        {/* Sidebar Navigation */}
        <div className="lg:col-span-3 glass-extreme border-cyan-500/30 shadow-[0_0_15px_rgba(0,240,255,0.05)] rounded-3xl p-4 space-y-1">
          {PANELS.map(p => <PanelItem key={p.id} panel={p} />)}
        </div>

      {/* Main Panel Content */}
      <div className="lg:col-span-9 glass-extreme border-cyan-500/30 rounded-3xl overflow-hidden shadow-2xl relative flex-col">
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-4 mb-8">
              <div className={`p-4 rounded-2xl bg-black/40 backdrop-blur-md border-white/5 ${PANELS.find(p => p.id === activePanel)?.color}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 auto-rows-min gap-6 [&>*:nth-child(1)]:col-span-1 lg:[&>*:nth-child(1)]:col-span-2">
              <div className="bg-black/30 border-cyan-500/30 shadow-[0_0_15px_rgba(0,240,255,0.05)] p-6 rounded-2xl">
              ...
              <div className="bg-black/30 border-cyan-500/30 shadow-[0_0_15px_rgba(0,240,255,0.05)] p-6 rounded-2xl">
                <h3 className="text-sm font-bold mb-4 flex items-center gap-2">
                  <History size={16} className="text-neon-cyan" /> Recent Events
                </h3>
                <div className="space-y-3 font-mono text-[10px]">
                  ...
                  <div className="text-neon-cyan">[00:01:15] Capturing truth-state for reality-twin</div>
        ...
        {/* Footer info */}
        <div className="p-4 bg-indigo-500/5 border-t border-cyan-500/30 shadow-[0_0_15px_rgba(0,240,255,0.05)] text-[10px] text-slate-500 font-bold flex justify-between">
...
const StatusLine = ({ label, value }) => (
  <div className="flex justify-between items-center text-xs">
    ...
    <span className="text-neon-cyan font-black">{value}</span>
  </div>
);
